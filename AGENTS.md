...
- `crates/`: Rust workspace crates — `server` (API + bins), `db` (SQLx models/migrations), `executors`, `services`, `utils`, `git` (Git operations), `api-types` (shared API types), `review` (PR review tool), `deployment`, `local-deployment`, `relay-control`, `relay-protocol`, `relay-ws`, `relay-tunnel-core`, `local-relay`, `ssh-tunnel`, `trusted-key-auth`, `relay-types`.
...
- Type checks: `pnpm run check` (frontend + backend Rust workspace) and `pnpm run backend:check` (backend Rust workspace only)
