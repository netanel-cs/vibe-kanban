---
title: "Agent Kanban MCP Server"
description: "Configure the Agent Kanban MCP server"
---

Agent Kanban exposes a local MCP (Model Context Protocol) server, allowing you to manage organisations, projects, issues, workspaces, and repositories from external MCP clients like Claude Desktop, Raycast, or even coding agents running within Agent Kanban itself.

<Note>
This page covers connecting **external MCP clients** to Agent Kanban's MCP server. For configuring MCP servers **within** Agent Kanban for your coding agents, see the [MCP Server Configuration](/integrations/mcp-server-configuration) guide.
</Note>

<Info>
Agent Kanban's MCP server is **local-only** - it runs on your computer and can only be accessed by applications installed locally. It cannot be accessed via publicly accessible URLs.
</Info>
...
This works if you're adding the Agent Kanban MCP server to any [supported coding agent](/supported-coding-agents) **within** Agent Kanban.

1. In Agent Kanban Settings, navigate to the "MCP Servers" page
2. In the "Popular servers" section, click on the Agent Kanban card
3. Click the `Save Settings` button

<Frame>
<img src="/images/vk-mcp-server-config.png" alt="MCP Servers configuration page showing how to add Agent Kanban MCP server" />
</Frame>
...
The Agent Kanban MCP server provides tools for managing organisations, projects, issues, workspaces, and task execution.
...
3. **Automatic Issue Generation**: Your MCP client will use the Agent Kanban MCP server to automatically create structured issues in your project
...
Your MCP client will use the `create_issue` tool to break this down into individual issues and add them to your Agent Kanban project automatically.
...
### Internal Coding Agents (Within Agent Kanban)

A powerful workflow involves using coding agents within Agent Kanban that are also connected to the Agent Kanban MCP server:
...
4. **Automatic Population**: The agent uses the MCP server to populate individual issues directly in the Agent Kanban interface
...
Raycast is a popular MCP client that can connect to Agent Kanban's MCP server. Here's how to configure it:
...
<Frame>
<img src="/images/vk-raycast-mcp-part-1.png" alt="Raycast MCP configuration - adding Agent Kanban server" />
</Frame>

Configure the Agent Kanban MCP server in Raycast by adding the server details.
</Tab>
...
Once configured, you'll see the Agent Kanban MCP server listed and ready to use in Raycast.
</Tab>
</Tabs>
