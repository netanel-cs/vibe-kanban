...
description: "Configure MCP (Model Context Protocol) servers to enhance your coding agents within Agent Kanban with additional tools and capabilities."
---


<Note>
This page covers configuring MCP servers **within** Agent Kanban for your coding agents. For connecting external MCP clients to Agent Kanban's MCP server, see the [Agent Kanban MCP Server](/integrations/vibe-kanban-mcp-server) guide.
</Note>

## Overview

MCP servers provide additional functionality to coding agents through standardized protocols. You can configure different MCP servers for each coding agent in Agent Kanban, giving them access to specialized tools like browser automation, access to remote logs, error tracking via Sentry, or documentation from Notion.

## Accessing MCP Server Configuration

1. Navigate to **Settings** in the Agent Kanban interface
...
Agent Kanban provides one-click installation for popular MCP servers. Click a card to insert a pre-configured MCP server into the JSON configuration.

<Frame>
<img src="/images/settings-popular-mcp-servers.png" alt="Popular MCP servers including Agent Kanban, Context7, Playwright, Exa, Chrome DevTools, and Dev Manager" />
</Frame>
...
<Check>
These changes update the global configuration file of the coding agent and will persist even if you stop using Agent Kanban.
</Check>
</Step>
</Steps>
