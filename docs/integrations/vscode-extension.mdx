...
description: "Complete guide to using the Agent Kanban extension with VSCode, Cursor, and Windsurf"
---

The Agent Kanban VSCode extension brings task management directly into your IDE, providing seamless integration with logs, diffs, and process monitoring. This extension works with VSCode and popular forks including Cursor and Windsurf.
...
<Card title="VSCode Marketplace" icon="download" href="https://marketplace.visualstudio.com/items?itemName=bloop.vibe-kanban">
  Install the official Agent Kanban extension for VSCode
</Card>
...
<Steps>
<Step title="Start a task in Agent Kanban">
...
<Step title="Launch IDE integration">
  ...
  <Note>
  The button text will reflect your editor choice configured in Agent Kanban settings.
  </Note>
</Step>
...
**Solution**: Ensure you're working within a worktree created by a Agent Kanban task.

<Warning>
The extension can only display task information when VSCode is opened in a directory that corresponds to an active Agent Kanban task worktree.
</Warning>

**Steps to resolve**:
1. Verify you opened VSCode through the "Open in [IDE]" button from a Agent Kanban task
...
**Problem**: The Agent Kanban extension doesn't appear in your IDE.
...
<Tip>
**Workflow Optimization**: Always start tasks from the Agent Kanban web interface before opening in your IDE to ensure proper context and worktree setup.
</Tip>
