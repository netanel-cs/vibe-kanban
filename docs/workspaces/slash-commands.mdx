...
The available slash commands depend on which coding agent you're using. Agent Kanban discovers and exposes the same commands you'd have in the agent's native CLI, so the experience matches what you're used to.

<Note>
Some built-in commands that require TUI interaction (such as `/model` or `/theme`) are not currently supported in Agent Kanban. Only non-interactive commands are available.
</Note>
...
When you select an agent, Agent Kanban automatically discovers the available slash commands. You'll see "Discovering commands..." in the typeahead while this happens.
